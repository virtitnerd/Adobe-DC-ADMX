...
column is the radio button / dropdown selection to make in GPMC or Intune - use this, not the raw registry value, to
configure the policy. Several policies write an **inverted** registry value (selecting **Enabled** writes `0`, not
`1`) - marked with ⚠ below. Getting this backwards silently fails the control.
...
| V-213136 | AADC-CN-001310 | CAT III | Welcome Screen must be disabled | Welcome Screen on Startup | Startup & Experience | **Enabled** ⚠ | `FeatureLockDown\cWelcomeScreen\bShowWelcomeScreen` | `0` | Computer |
